...
def tableListenercontinue(txt):
    ...
    try:
        ...
        logger.debug(str(processRow))
        oracleDaosys1 = OracleDao.OracleDao(logger, columnLookUp['usernamesys1'],columnLookUp['passwordsys1'],columnLookUp['jdbcurlsys1'])
        oracleDaosys2 = OracleDao.OracleDao(logger, columnLookUp['usernamesys2'],columnLookUp['passwordsys2'],columnLookUp['jdbcurlsys2'])
        if columnLookUp['sys1type']  == "DB" :
            ...
            compareDataForBatchApproach(sys1,sys2,dplistenDaoObj,diffTableDaoObj,processRow,columnLookUp['rowcount'],columnLookUp['taskid'],columnLookUp['runid'],"true",columnLookUp['tablewildcard'],None)
        else :
            
            logger.debug("hello")    
        dplistenDaoObj.updateStatus(columnLookUp['taskid'],columnLookUp['runid'],'DONE')
...
def tableListener():
    ...
    try:
        ...
        if columnLookUp['generate_metadata'] != None and columnLookUp['generate_metadata'] !='' and columnLookUp['generate_metadata'] =='true' :
            ...
            ansold=metaDataGenObj.generateMetaDataDifference(diffTablestructObj,sys1Meta,sys2Meta,columnLookUp['taskid'],'1' )
            done = time.time()
        ...
        if columnLookUp['starttsonlinesys1'] != None and columnLookUp['starttsonlinesys1'] !='' and len(columnLookUp['starttsonlinesys1']) >0 :            
            sys1=oracleDaosys1.readFromTriggerTable(columnLookUp['tablenamesys1'],columnLookUp['starttsonlinesys1'],columnLookUp['endtsonlinesys1'],columnLookUp['tablewildcard'])
            sys2=oracleDaosys2.readFromTriggerTable(columnLookUp['tablenamesys2'],columnLookUp['starttsonlinesys2'],columnLookUp['endtsonlinesys2'],columnLookUp['tablewildcard'])
            done = time.time()
            loadtime =int(( done - start))+1
        ...
        if columnLookUp['starttsonlinesys1'] != None and columnLookUp['starttsonlinesys1'] !='' and len(columnLookUp['starttsonlinesys1']) >0 :              
            ...
            sys1=oracleDaosys1.readClobTriggerTable(columnLookUp['tablenamesys1']+"_CLOB",columnLookUp['starttsonlinesys1'],columnLookUp['endtsonlinesys1'],columnLookUp['tablewildcard'])
            sys2=oracleDaosys2.readClobTriggerTable(columnLookUp['tablenamesys2']+"_CLOB",columnLookUp['starttsonlinesys2'],columnLookUp['endtsonlinesys2'],columnLookUp['tablewildcard'])
        ...
        if columnLookUp['starttssys1'] != None and columnLookUp['starttssys1'] !='' and len(columnLookUp['starttssys1']) >0 :   
            countsys1=oracleDaosys1.countRows(columnLookUp['tablenamesys1'],columnLookUp['starttssys1'],columnLookUp['endtssys1'])
            countsys2=oracleDaosys2.countRows(columnLookUp['tablenamesys2'],columnLookUp['starttssys2'],columnLookUp['endtssys2'])
            ...
            start = time.time()
            dplistenDaoObj.updateProcessTime(columnLookUp['taskid'],columnLookUp['runid'] ,loadtime,batchtime,comaparetime,reporttime)
            compareDataForBatchApproach(oracleDaosys1,oracleDaosys2,dplistenDaoObj,diffTableDaoObj,sys1,sys2,processRow,columnLookUp['rowcount'],columnLookUp['taskid'],columnLookUp['runid'],"false",columnLookUp['tablewildcard'],realtname,ignoretable,difftableStruct)
            done = time.time()
            comaparetime =int(( done - start))+1
            reporttime=1
            dplistenDaoObj.updateProcessTime(columnLookUp['taskid'],columnLookUp['runid'] ,loadtime,batchtime,comaparetime,reporttime)
...
def compareDataForOnline(dplistenDaoObj:DplistenDao.DplistenDao,diffTableDaoObj:DiffTableDao.DiffTableDao,tid,scndetssys1,scndetssys2,taskid,runid,realtname,ignoretable,difftableStruct):
    ...
    try:
        xmlProcessor = XmlProcessor.XmlProcessor(logger)
...
def compareDataForBatchOneCycle(dplistenDaoObj:DplistenDao.DplistenDao,diffTableDaoObj:DiffTableDao.DiffTableDao,tid,scndetssys1,scndetssys2,taskid,runid,realtname,ignoretable,difftableStruct):
    ...
    try:
        ...
        for i in range(1):
            t = threading.Thread(target=compareDataForOnlinethread,args=(dplistenDaoObj,diffTableDaoObj,tid,inrval,intriger, tab1, tab2, tnameConcattab1 , tnameConcattab2,tnamereversemap,taskid,runid,realtname))
...
def compareDataForBatchApproach(oracleDaosys1,oracleDaosys2,dplistenDaoObj:DplistenDao.DplistenDao,diffTableDaoObj:DiffTableDao.DiffTableDao,sys1,sys2,processRow,rowcount,taskid,runid,cont,tablewildcard,realtname,ignoretable,difftableStruct):
    ...
    try:    
        ...
        for i in range(len(sys1)):
            ...
            # if batch!= "BNK/AA.EOD.PROCESS" :
                # continue     
            mintime=sys1[i][1]
            maxtime=sys1[i][2]
            scndetssys1=oracleDaosys1.fbatchRangeSearch( processRow[14],mintime,maxtime,tablewildcard)
            ...
            for row in sys2 :
                if row[0]== batch :                     
                    ...
                    mintimesys2=row[1]
                    maxtimesys2= row[2]
            scndetssys2=oracleDaosys2.fbatchRangeSearch( processRow[21],mintimesys2,maxtimesys2,tablewildcard)
            if  scndetssys1 == None :
