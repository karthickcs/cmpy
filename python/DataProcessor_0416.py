...
def dproces(name,start,end, tab1, tab2, counttab1 , counttab2 , tnameConcattab1 , tnameConcattab2, tnamereversemap):
    try:
...
#########################################################################################
#################################################################################
def mainProgram(fname,rowcount):
    start = datetime.datetime.now(pytz.timezone('Asia/Kolkata'))
    print("Start --->:", start)
    tranDataTab1=readexceldata(fname,'Tab1','B2:C'+rowcount,'A2:A'+rowcount)
    tranDataTab2=readexceldata(fname,'Tab2','B2:C'+rowcount,'A2:A'+rowcount)
    #writeTojson('tab1.json',tranDataTab1)
    #writeTojson('tab2.json',tranDataTab2)
    tnameConcattab1=gettrantableNames(fname,'Tab1','B2:C'+rowcount,'A2:A'+rowcount)
    tnameConcattab2=gettrantableNames(fname,'Tab2','B2:C'+rowcount,'A2:A'+rowcount)
    ...
    triger=10
    for i in range(1400):
        t = threading.Thread(target=dproces,args=("T-"+str(i),rval,rval+triger, tranDataTab1, tranDataTab2, counttab1 , counttab2, tnameConcattab1 , tnameConcattab2, tnamereversemap))
        t.start()
...
mainProgram('input_full.xlsx','480')
